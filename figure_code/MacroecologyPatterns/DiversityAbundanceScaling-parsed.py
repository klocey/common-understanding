...
def figplot(x, y, xlab, ylab, fig, n):
    ...
    if n == 1: plt.ylim(-0.1, 0.95)
    elif n == 2: plt.ylim(0.0, 4.2)
    elif n == 3: plt.ylim(-1.6, 0.05)
    elif n == 4: plt.ylim(0.0, 2.2)
...
df = df2.replace([np.inf, -np.inf], np.nan).dropna()
    ...
    for mt in mts:
        for q in qs:
            ...
            if df2.shape[0] > 4:
                ...
                plt.savefig(mydir + '/results/figures/DiversityAbundanceScaling/DiversityAbundanceScaling-'+str(gr)+'-'+str(mt)+'-'+str(q)+'.png',
